function biomotion_human2_trial_free(p,state)
...
phase_coherence = ceil(p.trial.stimulus.phase_coherence*size(A,1)); %values to scramble
...
%******noise randomization/initialization*****
Holder = [];
PS_block = cell(70, 1);
for i = 1:length(PS_block)
    PS_block{i, 1} = coordvec;
    ...
    PS_block{i, 1}(2, 1, :) = PS_block{i, 1}(2, 1, :) + (rand(1)*1980 - (1980/2));
    PS_block{i, 1}(2, 2, :) = PS_block{i, 1}(2, 2, :) + (rand(1)*1980 - (1980/2));
    count = 0;
    if (i <= ceil(p.trial.stimulus.phase_coherence * length(PS_block)))
        for timer = 1:size(coordvec, 3)
                count = count + (1980 /(size(coordvec,3))) ;
                PS_block{i, 1}(2, 1, timer) = PS_block{i, 1}(2, 1, timer) - (count);
        end
    end
    %r = randi([1,2]);
%     if r == 1
...
coordvec = cat(1, coordvec, Holder);
