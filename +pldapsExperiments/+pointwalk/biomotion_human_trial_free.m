function biomotion_human_trial_free(p,state)
...
randox = rand(1, 500)*1980 - (1980/2); %bounds should be within all values in dotpos
randoy = rand(1, 500)*1980 - (1980/2); %same here, all possible values in ferret movement
randox = randox';
randoy = randoy';
...
%this will account for a full walk along the screen 
count = 0;
...
for iter = 0:5
    if p.trial.stimulus.dotdistance == 0
        for timer = 1:size(coordvec, 3)
            ...
            coordvec(:, 1, timer) = coordvec(:, 1, timer) - (count*2);
        end
...
coordvec = cat(1, coordvec, coordvec2);
