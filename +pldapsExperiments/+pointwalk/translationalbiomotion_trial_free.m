...
%number of Dots
p.trial.stimulus.nrDots = p.conditions{1}.nrDots;
%dot size
...
p.trial.stimulus.dotcolor=ones(3,(p.trial.stimulus.nrDots + size(coordvec, 1)))*255;
p.trial.stimulus.dotcolor(:,1:round(p.trial.stimulus.fractionBlack*(p.trial.stimulus.nrDots+size(coordvec, 1))))=0;
p.trial.stimulus.dotcolor=p.trial.stimulus.dotcolor(:,randperm((p.trial.stimulus.nrDots+size(coordvec,1))));
...
%compute nr frames
p.trial.stimulus.nrFrames=(size(coordvec, 3))*1000;
...
function showStimulus(p)
        ...
        if p.trial.stimulus.frameI<=p.trial.stimulus.nrFrames
            ...
            thedots = randpos;
            countercell = zeros(1, p.trial.stimulus.nrFrames);
            for j = 1:size(coordvec,3):p.trial.stimulus.nrFrames
                countercell(j:j+(size(coordvec, 3)-1)) = 1:size(coordvec, 3);
            end
            finaldots(1,:) = [thedots(1,:) herex(countercell(f),:)];
            finaldots(2,:) = [thedots(2,:) herey(countercell(f),:)];
