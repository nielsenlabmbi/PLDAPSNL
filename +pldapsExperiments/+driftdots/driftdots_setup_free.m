function p = driftdots_setup_free(p)
...
p.trial.pldaps.trialFunction='pldapsExperiments.driftdots.driftdots_trial_free';
...
%else
   ...
   % if length(nrDots) > 2
        c=generateCondList_sides(cond,side,'pseudo',ceil(500/1*2));
%     else
%         c=generateCondList(cond,side,'pseudo',ceil(500/1*2));
%     end
...
if length(cond)>1
    [A] = ndgrid(cond(end).direction); %,cond(end).dotCoherence(:));
else
[A] = ndgrid(cond.direction); %,cond.dotCoherence(:));
end
