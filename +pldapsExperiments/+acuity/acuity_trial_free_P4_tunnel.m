function acuity_trial_free_P4_tunnel(p,state)
    ...
    case p.trial.stimulus.states.START %trial started
     ...
     case p.trial.stimulus.states.MOVE %wait for ferret to cross midline
        if activePort==p.trial.stimulus.port.MIDDLE
            ...
            p.trial.stimulus.show=1;

            pds.LED.stimLEDOn(p);
        end    

    case p.trial.stimulus.states.STIMON
        if p.trialMem.removeStim==1
...
if ~isfield(p.trialMem,'removeStim')
...
if isfield(p.trial.stimulus,'fullField') && p.trial.stimulus.fullField == 1
    ...
    if strcmp(p.trial.stimulus.masktype,'gauss')
...
switch p.trial.userInput
    ...
    case 1 %left key
        p.trialMem.removeStim=0; %Stimulus will remain once ferret crosses exit beam (default)
        disp('Stimulus Remains when Exit beam crossed')
    case 2 %right key
