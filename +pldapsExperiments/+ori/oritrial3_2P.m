...
%             end
    ...
    case p.trial.stimulus.states.INCORRECT %incorrect port selected for stimulus
        if p.trial.stimulus.forceCorrect == 1 %must give correct response before ending trial
            ...
            %if the ferret takes too long to correct mistake, blank the
            %laser
            if p.trial.ttime > p.trial.stimulus.timeTrialFirstResp + p.trial.stimulus.extraTime
                if p.trial.triggerState ~=p.trial.daq.trigger.trialfinish
                    p = pds.daq_com.send_daq(p,p.trial.daq.trigger.trialfinish);
                    p = pds.sbserver.shutter2P(p,'0');
                    p.trial.triggerState = p.trial.daq.trigger.trialfinish;
                    p.trial.TriggerTrialTimeout = p.trial.ttime;
                end
            end
