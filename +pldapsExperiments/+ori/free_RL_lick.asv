     ...
     case 1
        switch p.trial.state
            case p.trial.stimulus.states.START %trial started
                ...
                if activePort==p.trial.stimulus.port.RIGHT %start port activated
       ...
       case 2
           switch p.trial.state
               case p.trial.stimulus.states.START %trial started
                    ...
                    if p.trial.ttime > p.trial.stimulus.baseline && p.trial.ports.position(p.trial.stimulus.side.LEFT)==0
                         pds.ports.movePort(p.trial.stimulus.side.LEFT,1,p);
                    end
                    
                    
                    if activePort==p.trial.stimulus.port.LEFT %start port activated
