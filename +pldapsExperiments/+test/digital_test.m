...
%             p.trial.stimulus.frameTrialLedOn = p.trial.iFrame;
        ...
        switch p.trial.stimulus.switchVAR
            ...
            %give reward
                ...
                if (status.newLogFrames > 0)
                    ...
                    for bit = 0:2:4               % easier to understand in binary
                        if (bitand(data, 2^bit) <= 0)
                            activePort = bit/2 + 1;
                            if activePort == p.trial.stimulus.port.RIGHT & activePort == p.trial.side
                                amount=p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.RIGHT);
                                pds.behavior.reward.give(p,amount,p.trial.behavior.reward.channel.RIGHT);
                            elseif activePort == p.trial.stimulus.port.LEFT & activePort == p.trial.side
    ...
    case p.trial.stimulus.states.STIMON %stimulus shown; port selected in response
        ...
        %check whether any port chose
        while ~KbCheck %worth checking whether we can drop this while loop (since the whole function is one?)
            Datapixx('RegWrRd');
            status = Datapixx('GetDinStatus');
            if (status.newLogFrames > 0)
                [data tt] = Datapixx('ReadDinLog');
