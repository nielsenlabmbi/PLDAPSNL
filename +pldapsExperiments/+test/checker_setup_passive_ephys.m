function p=checker_setup_passive_ephys(p)
%experiment setup file for a simple test experiment

%% basic definitions
p = pdsDefaultTrialStructureNL(p); 

%% set the trial function: the function that gets called for each frame state
p.trial.pldaps.trialFunction='pldapsExperiments.test.checker_passive_view_ephys';

%% set general parameters
p.trial.stimulus.forceCorrect = 1;

%% set timing parameters
p.trial.stimulus.duration.ITI = p.defaultParameters.stimulus.duration.ITI; %ITI in s


%% conditions:
cond.sides2match = [1 2];
side.par = 'sides2match';
side.match=[1 2];
cond.t_period = p.defaultParameters.stimulus.t_period;
cond.fullField = p.defaultParameters.stimulus.fullField; 
cond.block_size = 6;
c=generateCondList_sides(cond,side,'pseudo',20);


p.conditions = c;

p.trial.pldaps.finish = length(p.conditions);



%% display stats
% p.trialMem.stats.cond={'direction', 'type'}; %conditions to display
% [A,B] = ndgrid(cond(1).direction,[1:1:length(p.trial.stimulus.t_period)]);
% p.trialMem.stats.val = [A(:),B(:)]';
% nCond=size(p.trialMem.stats.val,2);
% 
% p.trialMem.stats.count.correct=zeros(1,nCond);
% p.trialMem.stats.count.incorrect=zeros(1,nCond);
% p.trialMem.stats.count.Ntrial=zeros(1,nCond);