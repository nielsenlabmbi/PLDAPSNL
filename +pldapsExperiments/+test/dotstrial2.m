...
switch p.trial.state
    ...
    case p.trial.stimulus.states.LICKDELAY
        switch p.trial.stimulus.switchVAR
            ...
            for i = 1:round(amount/p.trial.behavior.reward.pulseAmount)
                pds.behavior.reward.give(p,p.trial.behavior.reward.pulseAmount,p.trial.behavior.reward.channel.START);
...
%             end
            ...
            %give reward
                if activePort==p.trial.stimulus.port.LEFT
                    ...
                    for i = 1:round(amount/p.trial.behavior.reward.pulseAmount)
                        pds.behavior.reward.give(p,p.trial.behavior.reward.pulseAmount,p.trial.behavior.reward.channel.LEFT);
                        WaitSecs(p.trial.behavior.reward.pulseInt)
                    end
                ...
                elseif activePort==p.trial.stimulus.port.RIGHT
                    ...
                    for i = 1:round(amount/p.trial.behavior.reward.pulseAmount);
                        pds.behavior.reward.give(p,p.trial.behavior.reward.pulseAmount,p.trial.behavior.reward.channel.RIGHT);
                        WaitSecs(p.trial.behavior.reward.pulseInt)
                    end
                end
