function lesion_dots_trial_P7B(p,state)
%7B allows stimulus cohrence to be adjusted by key press, 
%not by staircase


%This phase INDUCES COHERENCE
...
switch p.trial.state
    ...
    case p.trial.stimulus.states.CORRECT %correct port selected for stimulus
        ...
        if p.trial.ttime > p.trial.stimulus.timeTrialFirstResp + p.trial.stimulus.duration.ITI
            ...
            if p.trial.stimulus.sSide==-1 %left
...
function p=trialSetup(p)
    ...
    if ~isfield(p.trialMem,'dotCohDefaultR')
        p.trialMem.dotCohDefaultR = p.trial.stimulus.dotCohDefaultR;
    end
    if ~isfield(p.trialMem,'dotCohDefaultL')
        p.trialMem.dotCohDefaultL = p.trial.stimulus.dotCohDefaultL;
    end
    ...
    if p.trial.stimulus.sSide==-1 %left
        ...
        if p.trialMem.stairL==0 %staircase off; default value
            ...
            p.trialMem.dotCohL=p.trialMem.dotCohDefaultL; 
        ...
        if p.trialMem.stairR==0
            p.trialMem.dotCohR = p.trialMem.dotCohDefaultR;
...
%         p.trialMem.stats.count.correct./p.trialMem.stats.count.Ntrial*100)))
    ...
    switch p.trial.userInput
        ...
        case 3 %up
            p.trialMem.dotCohDefaultL=p.trialMem.dotCohDefaultL + (p.trial.stimulus.delta_cohL);
            p.trialMem.dotCohDefaultR=p.trialMem.dotCohDefaultR + (p.trial.stimulus.delta_cohR);
            if p.trialMem.dotCohDefaultL > 1
                p.trialMem.dotCohDefualtL =1;
            end
            if p.trialMem.dotCohDefaultR > 1
                p.trialMem.dotCohDefualtR =1;
            end
            disp(['Coherence L/R increased to ' num2str(p.trialMem.dotCohDefaultL)])
        case 4 %down
            p.trialMem.dotCohDefaultL=p.trialMem.dotCohDefaultL - (p.trial.stimulus.delta_cohL);
            p.trialMem.dotCohDefaultR=p.trialMem.dotCohDefaultR - (p.trial.stimulus.delta_cohR);
            if p.trialMem.dotCohDefaultL < 0
                p.trialMem.dotCohDefualtL = 0;
            end
            if p.trialMem.dotCohDefaultR < 0
                p.trialMem.dotCohDefualtR = 0;
            end
            disp(['Coherence L/R decreased to ' num2str(p.trialMem.dotCohDefaultL)])
    end
