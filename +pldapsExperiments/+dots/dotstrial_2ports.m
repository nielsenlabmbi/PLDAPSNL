function dotstrial_2ports(p,state)
...
switch p.trial.state
    ...
    case p.trial.stimulus.states.STIMON %stimulus shown; port selected in response
        ...
        if p.trial.ttime > p.trial.stimulus.timeTrialStimOn + p.trial.stimulus.stimON && p.trial.ports.position(p.trial.ports.dio.channel.LEFT)==0 && p.trial.ports.position(p.trial.ports.dio.channel.MIDDLE)==0;
...
%             pds.ports.movePort([p.trial.ports.dio.channel.LEFT p.trial.ports.dio.channel.RIGHT],1,p);
        ...
        if activePort==p.trial.stimulus.port.LEFT | activePort==p.trial.stimulus.port.START
            %note time
            ...
            if correct==1
                ...
                if p.trial.side==p.trial.stimulus.side.MIDDLE
    ...
    case p.trial.stimulus.states.INCORRECT %incorrect port selected for stimulus
        if p.trial.stimulus.forceCorrect == 1 %must give correct response before ending trial
            ...
            if activePort==p.trial.stimulus.port.LEFT | activePort==p.trial.stimulus.port.START
                ...
                if correct==1 %now has chosen correct port
                    ...
                    if activePort==p.trial.stimulus.port.LEFT
                        ...
                        pds.behavior.reward.give(p,amount,p.trial.behavior.reward.channel.LEFT);
                    elseif activePort==p.trial.stimulus.port.START
                        amount=p.trial.behavior.reward.propAmtIncorrect*p.trial.behavior.reward.amount(p.trial.stimulus.rewardIdx.START);
                        pds.behavior.reward.give(p,amount,p.trial.behavior.reward.channel.START);
                    end
            ...
            %retract spouts
            if any(p.trial.ports.position)
                pds.ports.movePort([p.trial.ports.dio.channel.LEFT p.trial.ports.dio.channel.MIDDLE],0);
            end
