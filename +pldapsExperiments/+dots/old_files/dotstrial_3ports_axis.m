function dotstrial_3ports_axis(p,state)
...
%                 pds.datapixx.analogOutTime(0, 3, 0, p.trial.datapixx.dac.sampleRate);  
       ...
       if p.trial.ttime > p.trial.stimulus.timeTrialStimOn + p.trial.stimulus.stimON && p.trial.ports.position(p.trial.stimulus.side.LEFT)==0 && p.trial.ports.position(p.trial.stimulus.side.RIGHT)==0;
            ...
            if isfield(p.trial,'fracInstructTrue')
                if p.trial.fracInstructTrue | p.trial.stimulus.offset >= p.trial.stimulus.instructCutoff;
...
%direction
p.trial.stimulus.direction = p.conditions{p.trial.pldaps.iTrial}.reference + p.conditions{p.trial.pldaps.iTrial}.offset*p.conditions{p.trial.pldaps.iTrial}.rotation;
