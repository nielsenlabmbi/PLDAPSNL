...
if strcmp(randType,'pseudo')
    ...
    allConds = [];
    for i=1:nrBlocks
        %separate the different conditions into their rewarded sides
        signed = sign(combCond(1,:) - combCond(2,:));
        side1 = combCond(:,signed == 1); % test rewarded
        %side2 = combCond(:,signed == -1); % reference rewarded
        
        %shuffle within each side
        condPerm = [];
        for k = 1:length(side.match)
            condPerm = [condPerm; randperm(size(side1,2))];
        end
        
        %get rid of equal comparisons - we don't care
        combCond = combCond(:,~(signed == 0));
        
        %shuffle side assignment, in batches
        ...
        conditions = zeros(size(repmat(side1,1,length(side.match))));
        %find the side assignments
        for k = 1:length(side.match)
            sideIdx(k,:) = find(allsides == side.match(k));
            conditions(:,sideIdx(k,:)) = side1(:,condPerm(k,:));
        end
